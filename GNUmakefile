...
.PHONY: all
all: $(IMAGE_NAME).iso ECHO_DONE

.PHONY: ECHO_DONE
ECHO_DONE:
	@echo
	@echo
	@echo "  Build complete, kernel output is in ASNU/bin-x86_64/asnuk"
	@echo "  Generated .hdd/.iso file in the workspace root directory"
...
ifeq ($(ARCH),x86_64)
	...
	xorriso -as mkisofs -R -r -J -b boot/limine/limine-bios-cd.bin \
		...
		iso_root -o $(IMAGE_NAME).iso -quiet
	./limine/limine bios-install $(IMAGE_NAME).iso
endif
ifeq ($(ARCH),aarch64)
	cp limine/limine-uefi-cd.bin iso_root/boot/limine/
	cp limine/BOOTAA64.EFI iso_root/EFI/BOOT/
	xorriso -quiet -as mkisofs -R -r -J \
